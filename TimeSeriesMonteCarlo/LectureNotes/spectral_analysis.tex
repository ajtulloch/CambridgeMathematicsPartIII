...
\begin{thm}
  ...
  A scomplex valued function $\gamma(\cdot)$ is the autocovariance
...
\begin{thm}
  ...
  If $x_{1}, \dots, x_{n}$ are any real enumbers and $\omega_{k}$ is
...
\begin{thm}
  ...
  \item If $f(\lambda) > 0$for all $\lambda \in [-\pi, \pi]$ and if $0
    ...
    distribution ot a vector of independent and exponentially
...
\begin{thm}
  ...
  average estimator of the spectral denity $f$, then for $\lambda,
  \omega \in [0, \pi]$,
  \begin{enumerate}
...
\begin{defn}
  ...
  ARMIA($p,d,q$) process if $Y_{t} = (1 - B)^{d} X_{t}$ is a causal
  ARMA($p, q$) process
\end{defn}
...
\begin{thm}
  ...
  If there is a slowly decaying osciallatory  sample ACF, applying
  the operator $(1 - B + B^{2})$ can appleid to produce a aseries with
  a more rapidly autocorrelation function.
\end{thm}

\begin{defn}
  \label{sec:arima-processes-3}
  The Box-Cox transfomration is  a\textbf{variance-stabilizing
    transformation} - and should be used whenever the standard
  devaition increases \textbf{lienarly} with the mean.  The equation
  is
  \begin{equation}
    \label{eq:77}
    f_{\lambda}(U_{t}) =
