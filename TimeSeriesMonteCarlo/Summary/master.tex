...
For the causal \textsc{ARMA}($p, q$) process
\begin{equation}
  \label{eq:35}
...
\begin{thm}[Maximum Likelihood Estimators for \textsc{ARMA} processes]
...
\begin{thm}[Asyptotic Distribution of Maximum Likelihood Estimators]
  ...
  For a large sample from an \textsc{ARMA}($p, q$) process,
  \begin{align}
    \label{eq:64}
...
\begin{thm}[AICC of \textsc{ARMA}($p, q$) process]
...
\begin{thm}[AIC of \textsc{ARMA}($p, q$) process]
...
\begin{thm}[BIC of \textsc{ARMA}($p, q$) process]
  \label{defn:estimation_arma:13}
  \begin{align}
    \label{eq:69}
